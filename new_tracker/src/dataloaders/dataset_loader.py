...
class ImageDataset(Dataset):
    def __init__(
        ...
        if is_train:
            ...
            self._color_jitter_brightness = cfg["transform"]["train"]["color_jitter"][
                "brightness"
            ]
            self._color_jitter_contrast = cfg["transform"]["train"]["color_jitter"][
                "contrast"
            ]
            self._color_jitter_saturation = cfg["transform"]["train"]["color_jitter"][
                "saturation"
            ]
            self._color_jitter_hue = cfg["transform"]["train"]["color_jitter"]["hue"]
        else:
            self._color_jitter_p = cfg["transform"]["test"]["color_jitter"]["p"]
            self._color_jitter_brightness = cfg["transform"]["test"]["color_jitter"][
                "brightness"
            ]
            self._color_jitter_contrast = cfg["transform"]["test"]["color_jitter"][
                "contrast"
            ]
            self._color_jitter_saturation = cfg["transform"]["test"]["color_jitter"][
                "saturation"
            ]
    ...
    def __getitem__(self, index):
        ...
        for idx, img_path in enumerate(img_paths):
            ...
            if trans_input is None:
                ...
                for scale in self._out_scales:
                    trans_outputs[scale] = get_transform(
                        np.asarray(img), (out_w, out_h)
                    )
                    out_w = out_w // 2
                    out_h = out_h // 2

                if not self._is_train:
                    trans_input_inv = get_transform(
                        np.asarray(img), self._input_wh, inv=1
                    )
                    out_w, out_h = self._output_wh
                    for scale in self._out_scales:
                        trans_outputs_inv[scale] = get_transform(
                            np.asarray(img), (out_w, out_h), inv=1
                        )
                        out_w = out_w // 2
                        out_h = out_h // 2
            ...
            img = Image.fromarray(
                cv2.warpAffine(
                    np.array(img), trans_input, self._input_wh, flags=cv2.INTER_LINEAR
                )
            )
            imgs.append(img)
        ...
        for idx, (img_path, anno) in enumerate(zip(img_paths, annos)):
            ...
            for scale in self._out_scales:
                if visi:
                    ...
                    if self._model_name == "blurball":
                        hm = self._hm_generator(
                            (out_w, out_h), ct, angle, length, binary=binary
                        )
                        # hm = self._hm_generator(
                        #     (out_w, out_h), ct_int, angle, length, binary=binary
                        # )
                    else:
                        hm = self._hm_generator((out_w, out_h), ct, binary=binary)
                        # hm = self._hm_generator((out_w, out_h), ct_int, binary=binary)
                else:
                    if self._model_name == "blurball":
                        hm = self._hm_generator((out_w, out_h), (-1.0, -1.0), 0, 0)
                    else:
                        hm = self._hm_generator((out_w, out_h), (-1.0, -1.0))
        ...
        if self._rgb_diff:
            if len(imgs_t) != 2:
                raise ValueError(
                    "assume 2 images are input but {} given".format(len(imgs_t))
                )
