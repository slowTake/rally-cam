...
def inference_video(
    ...
    for cnt, img_path in enumerate(result_dict.keys()):
        ...
        if (gt is not None) and (evaluator_all is not None):
            evaluator_all.eval_single_frame(
                xy_pred, visi_pred, score_pred, center_gt.xy, center_gt.is_visible
            )
        ...
        if vis_frame_dir is not None:
            ...
            for cnt2, img_path2 in enumerate(result_dict.keys()):
                ...
                vis_gt = draw_frame(vis_gt, center=center_gt, color=color_gt, radius=2)
    ...
    # df["Frame"] = df.index
    # df.to_csv(osp.join(csv_dir, csv_dir.split("/")[-1] + ".csv"), index=False)
...
class VideosInferenceRunner(BaseRunner):
    ...
    def _run_model(self, model=None):
        ...
        for key, dataloader_and_gt in self._clip_loaders_and_gts.items():
            ...
            csv_dir = osp.join(self._output_dir, "{}_{}".format(match, clip_name))
            mkdir_if_missing(csv_dir)
            if self._vis_result:
